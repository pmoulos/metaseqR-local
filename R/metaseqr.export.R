...
#'
#' This function help build the output files of the metaseqr pipeline based on several elements produced during the pipeline execution.
#' It is intended for internal use and not available to the users.
#'
...
#' @param adj.p.mat a matrix of adjusted p-values, see the documentation of \code{\link{metaseqr}}.
#' @param sum.p a vector of combined p-values, see the documentation of \code{\link{metaseqr}}.
#' @param adj.sum.p a vector of adjusted combined p-values, see the documentation of \code{\link{metaseqr}}.
...
#' @return A list with three members: a data frame to be exported in a text file, a long string with the result in a html formatted
#' table (if \code{report=TRUE}) and the column names of the output data frame.
...
{	
	...
	if ("annotation" %in% export.what)
	{
		...
		if (report) export.html <- cbind(export.html,make.html.cells(gene.data,type="text"))
	...
	if ("p.value" %in% export.what)
	{
		...
		if (report) export.html <- cbind(export.html,make.html.cells(p.mat))
	...
	if ("adj.p.value" %in% export.what)
	{
		...
		if (report) export.html <- cbind(export.html,make.html.cells(adj.p.mat))
	...
	if ("meta.p.value" %in% export.what && length(statistics)>1) # Otherwise it does not exist
	{
		...
		if (report) export.html <- cbind(export.html,make.html.cells(sum.p))
	...
	if ("adj.meta.p.value" %in% export.what && length(statistics)>1)
	{
		...
		if (report) export.html <- cbind(export.html,make.html.cells(adj.sum.p))
