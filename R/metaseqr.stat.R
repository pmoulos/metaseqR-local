...
stat.deseq <- function(object,sample.list,contrast.list=NULL,stat.args=NULL,norm.args=NULL) {
	...
	switch(class(object),
		...
		list = { # Has been normalized with NBPSeq and main method was "nbpseq"
			cds <- newCountDataSet(as.matrix(round(sweep(object$counts,2,object$norm.factors,"*"))),the.design$condition)
			sizeFactors(cds) <- rep(1,ncol(cds))
			cds <- estimateDispersions(cds,method=stat.args$method,sharingMode=stat.args$sharingMode)
		},
		nbp = { # Has been normalized with NBPSeq and main method was "nbsmyth"... Jesus...
			cds <- newCountDataSet(as.matrix(round(object$pseudo.counts)),the.design$condition)
			sizeFactors(cds) <- rep(1,ncol(cds))
			cds <- estimateDispersions(cds,method=stat.args$method,sharingMode=stat.args$sharingMode)
		}
	)
...
#' @param contrast.list a named structured list of contrasts as returned by \code{\link{make.contrast.list}} or just the vector of
#' contrasts as defined in the main help page of \code{\link{metaseqr}}.
#' @param stat.args a list of edgeR statistical algorithm parameters. See the result of \code{get.defaults("statistics",} \code{"edger")}
...
#' @param contrast.list a named structured list of contrasts as returned by \code{\link{make.contrast.list}} or just the vector of
#' contrasts as defined in the main help page of \code{\link{metaseqr}}.
#' @param stat.args a list of edgeR statistical algorithm parameters. See the result of \code{get.defaults("statistics",} \code{"limma")}
#' for an example and how you can modify it.
#' @return A named list of p-values, whose names are the names of the contrasts.
#' @author Panagiotis Moulos
#' @export
...
stat.noiseq <- function(object,sample.list,contrast.list=NULL,stat.args=NULL,norm.args=NULL,gene.data=NULL,log.offset=1) {
	...
	switch(class(object),
		...
		list = { # Has been normalized with NBPSeq and main method was "nbpseq"
			ns.obj <- NOISeq::readData(
				data=as.matrix(round(sweep(object$counts,2,object$norm.factors,"*"))),
				length=gene.length,
				gc=gc.content,
				chromosome=gene.data[,1:3],
				factors=data.frame(class=classes),
...
stat.bayseq <- function(object,sample.list,contrast.list=NULL,stat.args=NULL,norm.args=NULL,libsize.list=NULL) {
	...
	switch(class(object),
		...
		list = {
			bayes.data <- as.matrix(round(sweep(object$counts,2,object$norm.factors,"*")))
		},
		nbp = {
...
#'
#' This function is a wrapper over NBPSeq statistical testing. It accepts a matrix of normalized gene counts or an S4 object specific
#' to each normalization algorithm supported by metaseqr.
#'
#' @param object a matrix or an object specific to each normalization algorithm supported by metaseqr, containing normalized counts.
#' Apart from matrix (also for NOISeq), the object can be a SeqExpressionSet (EDASeq), CountDataSet (DESeq), DGEList (edgeR) or list
#' (NBPSeq).
#' @param sample.list the list containing condition names and the samples under each condition.
#' @param contrast.list a named structured list of contrasts as returned by \code{\link{make.contrast.list}} or just the vector of
#' contrasts as defined in the main help page of \code{\link{metaseqr}}.
...
#' \code{replace=TRUE else} \code{replace=FALSE}.
...
#' # Not yet available
#'}
meta.perm <- function(counts,sample.list,contrast,statistics,stat.args,nperm=10000,weight=rep(1,ncol(counts)),
	select=c("min","max","weight"),replace="auto",multic=FALSE) {
	...
	wapply(multic,relist,function(x,s,cnt,r) {
		tmp <- make.permutation(counts,sample.list,contrast) # Write in utils
		#resample <- sample(1:ncol(x),ncol(x),replace=r)
		#tmp <- counts[,resample]
		#cnts <- strsplit(contrast,"_vs_")[[1]]
		#virtual.contrasts <- length(cnts)
		for (alg in statistics) {
			disp("    running permutation tests with: ",alg)
			switch(alg,
				deseq = {
					p.list <- stat.deseq(tmp,sample.list,contrast.list,stat.args[[alg]],norm.args)
				},
				edger = {
					p.list <- stat.edger(tmp,sample.list,contrast.list,stat.args[[alg]])
				},
				noiseq = {
					p.list <- stat.noiseq(tmp,sample.list,contrast.list,stat.args[[alg]],norm.args,gene.data.expr,log.offset)
				},
				bayseq = {
					p.list <- stat.bayseq(tmp,sample.list,contrast.list,stat.args[[alg]],norm.args,libsize.list)
				},
				limma = {
					p.list <- stat.limma(tmp,sample.list,contrast.list,stat.args[[alg]])
				},
				nbpseq = {
					p.list <- stat.nbpseq(tmp,sample.list,contrast.list,stat.args[[alg]],norm.args,libsize.list)
				}
			)
		}
	},statistics,contrast,replace)
