		...
		if (length(con)==2) {
			...
			if (stat.args$replicates %in% c("technical","no"))
				...
				res <- noiseqbio(ns.obj,k=log.offset,norm="n",
					nclust=stat.args$nclust,factor=stat.args$factor,lc=stat.args$lc,
					conditions=stat.args$conditions,r=stat.args$r,adj=stat.args$adj,
					a0per=stat.args$a0per,cpm=stat.args$cpm,
			...
			warnwrap(
				paste("NOISeq differential expression algorithm does not support ",
					"multi-factor designs (with more than two conditions to be ",
					"compared)! Switching to DESeq for this comparison:",con.name))
...
stat.nbpseq <- function(object,sample.list,contrast.list=NULL,stat.args=NULL,
	...
	for (con.name in names(contrast.list)) {
		...
		if (length(con)==2) {
			#if (stat.args$main.method=="nbpseq") {
			#	dispersions <- estimate.dispersion(nb.data,model.matrix(~classes),
			#		method=stat.args$method$nbpseq)
			#	res <- test.coefficient(nb.data,dispersion=dispersions,
			#		x=model.matrix(~classes),beta0=c(NA,0),tests=stat.args$tests,
			...
			warnwrap(
				paste("NBPSeq differential expression algorithm does not support",
					"ANOVA-like designs with more than two conditions to be compared!",
					"Switching to DESeq for this comparison:",con.name))
			cds <- newCountDataSet(nb.data$counts,data.frame(condition=unlist(con),
				row.names=names(unlist(con))))
			sizeFactors(cds) <- rep(1,ncol(cds))
			cds <- estimateDispersions(cds,method="blind",sharingMode="fit-only")
			fit0 <- fitNbinomGLMs(cds,count~1)
