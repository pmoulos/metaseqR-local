...
approaches. For example, packages using the negative binomial distribution to model the null hypotheses (_DESeq_, _edgeR_) or packages
using Bayesian statistics (_baySeq_, _NBPSeq_). In addition, packages specialized to RNA-Seq data normalization have also been developed
(_EDASeq_). The package _metaseqr_ tries to provide an interface to several algorithms, similar to the recent _TCC_ package. However,
it is much simpler to use than _TCC_, incoporates more algorithms for normalization and statistical analysis and builds a full report
with several interactive and non-interactive diagnostic plots so that the users can easily explore the results and have whatever they
need for this part of their research in one place. Most importantly, metaseqr provides an interface for RNA-Seq data meta-analysis by
providing the ability to use different algorithms for the statistical testing part and combining the p-values using popular published
methods (e.g. Fisher's method, Whitlock's method) and two package-specific methods (intersection, union of statistically significant
results). In the future, more algorithms will be incorporated in the package, with more diagnostic plots and more examples. This initial
vignette contains just this introductory text and reference to some examples in the package documentation, which we believe that at
