...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    is constructed for each gene, containing the minimum p-value resulted from t
    he applied statistical tests (more lose).
    ...
    the applied statistical tests (more strict).
    \item In the third variant (\lstinline{dperm.weight}), an initial p-value
